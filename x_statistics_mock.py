...
def plot_pie_book_ratings(df: pd.DataFrame) -> None:
    ...
    df_latest_ratings['rating'] = df_latest_ratings['average_rating'].apply(
        convert_to_range)
    df_rating_counts = df_latest_ratings['rating'].value_counts().reset_index()
...
def mock_authors():
    return pd.DataFrame({
        'author_name': ['Suzanne Collins', 'J.K. Rowling'],
        'author_id': [1, 2]
    })


def mock_author_data(selected_author: str):
    start_date = datetime(2023, 1, 1)
    dates = [start_date + timedelta(days=i * 30) for i in range(12)]
    if selected_author == "Suzanne Collins":
        shelved_counts = [150 + i * (30 + randint(0, 10))
                          for i in range(len(dates))]
        rating_counts = [12 + i * (4 + randint(0, 5))
                         for i in range(len(dates))]
        average_ratings = [3.1 + i * (0.04) for i in range(len(dates))]
        author_id = 1
    else:
        shelved_counts = [500 + i * 80 for i in range(len(dates))]
        rating_counts = [50 + i * 6 for i in range(len(dates))]
        average_ratings = [4.2 - i * 0.02 for i in range(len(dates))]
        author_id = 2
    return pd.DataFrame({
        'date_recorded': dates,
        'shelved_count': shelved_counts,
        'rating_count': rating_counts,
        'average_rating': average_ratings,
        'author_name': [selected_author] * len(dates),
        'author_id': [author_id] * len(dates)
    })


def mock_author_books(selected_author: str):
    start_date = datetime(2023, 1, 1)
    dates = [start_date + timedelta(days=i * 30) for i in range(12)]
    if selected_author == "Suzanne Collins":
        book_titles = ['The Hunger Games', 'Catching Fire', 'Mockingjay']
        base_ratings = [3.8, 4.0, 4.2]
    else:
        book_titles = ['Harry Potter and the Philosopher\'s Stone',
                       'Harry Potter and the Chamber of Secrets', 'Harry Potter and the Prisoner of Azkaban']
        base_ratings = [4.4, 4.5, 4.6]
    data = []
    for idx, title in enumerate(book_titles):
        for i, date in enumerate(dates):
            data.append({
                'book_title': title,
                'date_recorded': date,
                'rating_count': 20 + idx * 5 + i * 4,
                'average_rating': base_ratings[idx] + (-0.05 + i * 0.005),
                'review_count': 5 + idx * 2 + i,
                'book_id': idx + 1
            })
...
if __name__ == "__main__":
    ...
    author_data = mock_author_data(selected_author)
    author_books = mock_author_books(selected_author)
